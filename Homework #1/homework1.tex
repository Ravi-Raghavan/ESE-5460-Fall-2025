...
\]
and is guaranteed to be \textbf{\underline{non-empty}}, provided that $\mu \in \operatorname{int}(\operatorname{dom}\varphi)$. \\

\noindent \textit{Note: Since the problem statement does not specify, I assume that the domain of $\varphi$ is $\mathbb{R}^n$} \\
\noindent \textit{Note: If $\mu$ lies on the boundary of the domain of $\varphi$, the subgradient inequality is trivial, as the right-hand side can become arbitrarily negative while $\varphi$ remains finite, implying that subgradients exist at such boundary points. For practicality, and to avoid dealing with $\mu$ being effectively infinite, we assume for the remainder of the proof that $\mu \in \operatorname{int}(\operatorname{dom}\varphi)$.} \\
